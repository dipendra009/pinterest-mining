...
function dataset:__init(...)
   ...
   for i=1,#self.classes do
      ...
      local length = tonumber(sys.fexecute(wc .. " -l '"
                                              ...
                                              .. cut .. " -f1 -d' '"))
      if length == 0 then
         error('Class has zero samples ' .. self.classes[i] .. wc .. " -l '"
                                              .. classFindFiles[i] .. "' |"
                                              .. cut .. " -f1 -d' '")
      else
...
function dataset:sample(quantity)
   ...
   elseif self.samplingMode == 'random' then
     local rand_idx = torch.random(1, #self.imagePath)
     local imgpath = ffi.string(torch.data(self.imagePath[rand_idx]))
