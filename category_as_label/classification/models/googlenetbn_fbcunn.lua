...
local function inception(input_size, config)
   ...
   if config[1][1] ~= 0 then
      ...
      conv1:add(nn.SpatialConvolutionCuFFT(input_size, config[1][1],1,1,1,1)):add(nn.SpatialBatchNormalization(config[1][1],1e-3)):add(cudnn.ReLU(true))
      concat:add(conv1)
   end

   local conv3 = nn.Sequential()
   conv3:add(nn.SpatialConvolutionCuFFT(  input_size, config[2][1],1,1,1,1)):add(nn.SpatialBatchNormalization(    config[2][1],1e-3)):add(cudnn.ReLU(true))
   ...
   conv3xx:add(nn.SpatialConvolutionCuFFT(  input_size, config[3][1],1,1,1,1)):add(nn.SpatialBatchNormalization(    config[3][1],1e-3)):add(cudnn.ReLU(true))
   ...
   if config[4][2] ~= 0 then
      pool:add(nn.SpatialConvolutionCuFFT(input_size, config[4][2],1,1,1,1)):add(nn.SpatialBatchNormalization(config[4][2],1e-3)):add(cudnn.ReLU(true))
   end
   concat:add(pool)
...
function createModel(nGPU)
   ...
   features:add(nn.SpatialConvolutionCuFFT(64,64,1,1)):add(nn.SpatialBatchNormalization(64,1e-3)):add(cudnn.ReLU(true))
   ...
   features:add(cudnn.SpatialConvolution(576,576,2,2,2,2)):add(nn.SpatialBatchNormalization(576,1e-3))
   ...
   main_branch:add(cudnn.SpatialConvolution(1024,1024,2,2,2,2)):add(nn.SpatialBatchNormalization(1024,1e-3))
   ...
   aux_classifier:add(nn.SpatialConvolutionCuFFT(576,128,1,1,1,1)):add(nn.SpatialBatchNormalization(128,1e-3))
